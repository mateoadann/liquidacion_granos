        ...
        {query.data && (
          <>
            ...
            <DataRow
              label="Localidad"
              value={
                [query.data.localidad, query.data.provincia].filter(Boolean).join(", ") || "-"
              }
            />
...
export function DatosLimpiosSection({ rawData, datosLimpios, taxpayerId }: DatosLimpiosProps) {
    ...
    <div className="space-y-4">
      <div className="bg-slate-100 border border-slate-300 p-4 rounded-lg">
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p className="text-sm text-slate-600 mt-1">
              Tipo de operación:{" "}
              {descOrCode(data, "descTipoOperacion", "codTipoOperacion", "Desconocido")}
            </p>
        ...
        <div className="grid grid-cols-2 gap-4 mt-4">
          <PersonaCard
            title="COMPRADOR"
            cuit={data["cuitComprador"] as string | null}
            taxpayerId={taxpayerId}
          />
          <PersonaCard
            title="VENDEDOR"
            cuit={data["cuitVendedor"] as string | null}
            taxpayerId={taxpayerId}
          />
        </div>
      </div>
...
export function AjusteLimpiosSection({ data }: { data: Record<string, unknown> }) {
