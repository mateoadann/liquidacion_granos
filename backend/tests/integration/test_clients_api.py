...
def _create_client(client, **overrides):
    response = client.post("/api/clients", json=_base_payload(**overrides))
    assert response.status_code == 201
    return response.get_json()


def test_create_client_ok(client):
    response = client.post("/api/clients", json=_base_payload())
...
def test_create_client_duplicate_cuit_returns_409(client):
...
def test_patch_without_clave_fiscal_keeps_has_clave_fiscal(client):
...
def test_patch_with_clave_fiscal_reencrypts(app, client):
    created = _create_client(client)
...
def test_logical_delete_sets_activo_false(app, client):
    created = _create_client(client)
    client_id = created["id"]
...
def test_upload_certificates_valid_pair(client, cert_pair_bytes):
    created = _create_client(client)
...
def test_upload_certificates_invalid_pair_returns_422(
    client, cert_pair_bytes, mismatched_private_key_bytes
):
    created = _create_client(client)
...
def test_validate_config_incomplete(client):
    created = _create_client(client)

    response = client.post(f"/api/clients/{created['id']}/validate-config")
...
def test_validate_config_complete(client, cert_pair_bytes):
    created = _create_client(client)
    cert_bytes, key_bytes = cert_pair_bytes
